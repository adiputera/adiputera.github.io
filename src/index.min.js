...
function updateNotifyUI(){const isIosDevice=isIos();const isStandalone=isInStandaloneMode();if(typeof Notification==="undefined"){if(isIosDevice&&!isStandalone){notifyButton.textContent="ℹ️ Add to Home Screen to enable notifications on iOS"}else{notifyButton.textContent="⚠️ Notifications not supported on this browser"}
notifyButton.disabled=!0;notifyButton.style.cursor="default";return}
...
const vapidPublicKey="BDQm2phIYp4W1xoVrwisbYBDp-GbT5_kbBjEYQXtoFnsftS_kr05oXD6yBTr9nYIyAu2p6kgAp4VMm8J01IRLII";function askNotificationPermission(){notifyButton.disabled=!0;if(!("Notification" in window)){notifyButton.textContent="❌ Notifications not supported in this browser";return}
notifyButton.textContent="🔄 Subscribing...";Notification.requestPermission().then(permission=>{if(permission==="granted"){navigator.serviceWorker.getRegistration().then(registration=>{if(!registration){return navigator.serviceWorker.register("/sw.js")}
return registration}).then(registration=>{if(!registration){console.error("Service Worker registration failed.");updateNotifyUI();return}
...
fetch('https://neon-bombolone-54f610.netlify.app/api/subscribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(subscription)});registration.showNotification("✅ You're subscribed!",{body:"I don't expect anyone would press that button, but you’ll be notified when I'm open to new roles.",icon:"images/512.png",badge:"images/badge.png"});updateNotifyUI()}).catch(error=>{console.error("Failed to subscribe:",error);updateNotifyUI()})}).catch(error=>{console.error("Service Worker registration not found:",error);updateNotifyUI()})}else if(permission==="denied"||permission==="default"){updateNotifyUI()}})}
