...
fetch('https://neon-bombolone-54f610.netlify.app/api/save-subscription',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(subscription)});registration.showNotification("✅ You're subscribed!",{body:"You’ll be notified when Yusuf is open to new roles.",icon:"images/512.png",badge:"images/badge.png"});updateNotifyUI()}).catch(error=>{console.error("Failed to subscribe:",error);updateNotifyUI()});clearInterval(interval)}).catch(error=>{console.error("Service Worker registration not found:",error);updateNotifyUI();clearInterval(interval)})}else if(permission==="denied"){updateNotifyUI();clearInterval(interval)}else if(Date.now()-start>=maxWait){updateNotifyUI();clearInterval(interval)}},intervalTime)}
