...
function askNotificationPermission(){if(!("Notification" in window)){notifyButton.textContent="❌ Notifications not supported in this browser";notifyButton.disabled=!0;return}
Notification.requestPermission();waitForNotificationPermission()}
function waitForNotificationPermission(maxWait=5000,intervalTime=500){const start=Date.now();const interval=setInterval(()=>{const permission=Notification.permission;if(permission==="granted"){new Notification("✅ You're subscribed!",{body:"You’ll be notified when Yusuf is open to new roles.",icon:"images/avatar.png",badge:"images/monogram-ya.png"});updateNotifyUI();clearInterval(interval)}else if(permission==="denied"){updateNotifyUI();clearInterval(interval)}else if(Date.now()-start>=maxWait){updateNotifyUI();clearInterval(interval)}},intervalTime)}
