if("serviceWorker" in navigator){navigator.serviceWorker.register("sw.js").then(registration=>{console.log("ServiceWorker successfully registered!");console.log(registration)}).catch(error=>{console.log("ServiceWorker registration failed!");console.log(error)})}
...
const notifyButton=document.getElementById("notify-btn");function updateNotifyUI(){if(Notification.permission==="granted"){notifyButton.textContent="✅ You're already subscribed!";notifyButton.disabled=!0;notifyButton.style.cursor="default"}}
function askNotificationPermission(){if(!("Notification" in window)){alert("This browser does not support notifications.");return}
